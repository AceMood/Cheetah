...
function connect() {
    ...
    client.log('requested connection to '.debug, client.id, client.path);
    if (!this.path) {
        client.log('===============error: '.error, client.id,' client has not specified socket path it wishes to connect to.'.error);
        return;
    }
    ...
    if (!client.port) {
        ...
        if (!client.config.tls) {
            client.log('Connecting client via TCP to'.debug, client.path ,client.port);
            client.socket = net.connect(
              {
                  port: client.port,
                  host: client.path
            ...
            client.log('Connecting client via TLS to'.debug, client.path ,client.port,client.config.tls);
    ...
    client.socket.on(
      ...
      function connectionMade() {
          ...
          client.log('=================== connect, retrying reset');
      }
    );
    ...
    client.socket.on(
      ...
      function connectionClosed() {
          client.log('=====================connection closed'.notice ,client.id , client.path,
